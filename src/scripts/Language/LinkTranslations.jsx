...
const LinkOption = ({ postId, refetchLangs, posts, source }) => {
  ...
  return (<div style={{ display: 'flex', justifyContent: 'space-between', border: '1px solid #e0e0e0', padding: '8px'}}>
    ...
    <Button variant='primary' isDestructive onClick={onLink} disabled={isLoading || isSuccess} style={{ height: 'min-content', margin: 'auto 0 auto 32px'}}>
      {!isSuccess && 'Link'}
      {isSuccess && 'Linked'}
    </Button>
  </div>)
...
const LinkTranslations = ({ postId, refetchLangs }) => {
  ...
  return (<>
    { isModalOpen && (
      <Modal title='Link to existing posts:' isOpen={isModalOpen} onRequestClose={closeModal}>
        <div style={{ display: 'grid', flexWrap: 'wrap', padding: '20px', gap: '8px' }}>
          <Notice status='warning' isDismissible={false} politeness='polite'> You will unlink from the post's current translations if you link to another.</Notice>
          {isLoading && 'Loading...'}
          {isError && 'ERROR!!!'}
        ...
        { page > 1 && <Button variant='secondary' onClick={previousPage}>Previous Page</Button> }
        <b style={{ padding: '10px' }}>{page}</b>
        { page < totalPages && <Button variant='secondary' onClick={nextPage}>Next Page</Button> }
        </div>
      </Modal>
    ) }
